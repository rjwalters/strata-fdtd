...
export function useDataWorker(): UseDataWorkerReturn {
  ...
  const downsample = useCallback(
    async (
      ...
      if (!isAvailableRef.current) {
        ...
        const { downsamplePressure } = await import("../lib/performance");
  ...
  const filter = useCallback(
    async (
      ...
      if (!isAvailableRef.current) {
        // Fallback to main thread
        const { filterByThreshold } = await import("../lib/performance");
  ...
  const downsampleAndFilter = useCallback(
    async (
      ...
      if (!isAvailableRef.current) {
        // Fallback to main thread
        const { downsamplePressure, filterByThreshold } = await import("../lib/performance");
